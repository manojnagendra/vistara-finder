...
def get_route_flights(from_code, to_code):
    ...
    cache_path = os.path.join(CACHE_DIR, f"{from_code}_{to_code}.json")
    ...
    # Cache miss - Scrape FlightMapper
    ...
    print(f"Cache miss for {from_code}-{to_code}. Fetching from FlightMapper...", file=sys.stderr)
    try:
        response = requests.get(url, headers=headers, timeout=10)
        if response.status_code != 200:
            print(f"FlightMapper returned status {response.status_code} for {from_code}-{to_code}", file=sys.stderr)
            return []
            
        # Check if we were redirected to a generic route page (non-Air India)
        if "Air_India_AI_" not in response.url and "/airline/AI" not in response.text:
            print(f"Redirected to non-Air India page for {from_code}-{to_code}", file=sys.stderr)
            # Cache empty list to avoid repeating bad queries
        ...
        if not flightlist_div:
            print(f"No flightlist div found for {from_code}-{to_code}", file=sys.stderr)
            return []
        ...
        # Cache results
        with open(cache_path, 'w') as f:
            json.dump(parsed_flights, f, indent=2)
            
        print(f"Successfully scraped and cached {len(parsed_flights)} flights for {from_code}-{to_code}", file=sys.stderr)
...
class VistaraFinderHandler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        # Support /vistara-finder prefix and default index serving
        if self.path.startswith('/vistara-finder/'):
            self.path = self.path.replace('/vistara-finder/', '/', 1)
        elif self.path == '/vistara-finder':
            self.path = '/'
            
        super().do_GET()

if __name__ == '__main__':
    # Change working directory to server file directory
    os.chdir(os.path.dirname(os.path.abspath(__file__)))
    
    with socketserver.TCPServer(("", PORT), VistaraFinderHandler) as httpd:
        print(f"ex-Vistara Finder custom server running on port {PORT}...", file=sys.stderr)
        try:
            httpd.serve_forever()
